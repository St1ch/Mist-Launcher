		...
		setup() {
			...
			const showWarning = () => {
				popupManager.addPopupNotification({
					...
					text: "Mist Launcher v2.1.0 is available now! Since you're on a metered network, we didn't automatically download it.",
					type: 'warning',
					autoCloseMs: null,
					buttons: [
						{
			...
			const showInfo = () => {
				popupManager.addPopupNotification({
					...
					text: 'Mist Launcher v2.1.0 has finished downloading. Reload to update now.',
					type: 'info',
					buttons: [
						{
							label: 'Reload',
